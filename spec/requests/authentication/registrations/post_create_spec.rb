...
describe 'Registration POST #create', type: :request do
  it 'increases user count by 1 when successful' do
    expect { post account_registration_path, params: { account: attributes_for(:account) } }
      .to change(Account, :count).by(1)
  end

  it 'does not change user count when unsuccessful' do
    expect { post account_registration_path, params: { account: { email: '' } } }
      .not_to change(Account, :count)
  end

  context 'empty submission' do
    before(:all) { post account_registration_path, params: { account: { email: '' } } }
